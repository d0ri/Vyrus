...
public class Vorspann : MonoBehaviour {
	...
	void Update () {
		...
		if(!movie.isPlaying) SceneManager.LoadScene(lev);   //statt Application.LoadLevel

		if (Input.GetKeyDown (KeyCode.Space)) {  //wenn Leertaste gedrückt wird, wenn Film läuft

			SceneManager.LoadScene(lev);                                      //läd nächsten level
		}
